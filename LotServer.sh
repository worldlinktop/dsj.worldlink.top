...
echo "            ======================================================";
echo "            |                    serverSpeeder                   |";
echo "            |                  安装遇到提示全按y      for Linux  |";
echo "            |----------------------------------------------------|";
echo "            |Linux TCP加速工具 LotServer(锐速母公司) 一键安装脚本|";
echo "            | WorldLink大世界搬运dsj.worldlink.top  -- By .Vicer |";
echo "            ======================================================";
echo "";
rootness;
cd /root
...
[ "$(du -b /appex/etc/apx.lic |awk '{ print $1 }')" -ne '152' ] && Unstall && echo "错误！ 我无法为您生成Lic，请稍后再试！ " && exit 1
...
[ ! -f /root/appex.zip ] && Unstall && echo "错误，找不到appex.zip! " && exit 1
mkdir -p /root/appex
